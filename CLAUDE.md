...
# Run a single test
.venv/bin/pytest tests/test_summary.py::test_summary_with_cycle_times -v
```
...
- **`stats.py`** — Analysis layer. Takes a file path, uses `reader.py` to iterate records, and returns a `LogSummary` dataclass. This is where new analysis logic should go.
